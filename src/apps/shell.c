...
void shell_main()
{
    ...
    while (1)
    {
        ...
        if (strncmp(cmd_line, "ls", 2) == 0)
        {
            ...
            while (*p == ' ') p++;
            if (*p == '\0') flist(c_path);
            else { merge_path(p, arg_path); flist(arg_path); }
        }
        // cd
        else if (strncmp(cmd_line, "cd", 2) == 0)
        {
            ...
            if (strcmp(p, "..") == 0) {
                ...
                if (open(arg_path) != -1) strcpy(c_path, arg_path);
                else printf_simple("Error: Directory not found.\n");
            }
        }
        // cat
        else if (strncmp(cmd_line, "cat", 3) == 0)
        {
            ...
            uint8_t buf[512];
            memset(buf, 0, 512);
            if (fread(arg_path, buf, 511) >= 0) { printf_simple((char*)buf); printf_simple("\n"); }
            else printf_simple("Error: Cannot read file.\n");
        }
        // mkdir
        else if (strncmp(cmd_line, "mkdir", 5) == 0)
        {
            char *p = cmd_line + 5;
            while (*p == ' ') p++;
            if (*p == '\0') printf_simple("Usage: mkdir <dirname>\n");
            else {
                merge_path(p, arg_path);
                if (mkdir(arg_path) == 0) printf_simple("Directory created.\n");
                else printf_simple("Error: Failed to create directory.\n");
            }
        }
        // write
        ...
        else if (strncmp(cmd_line, "append", 6) == 0)
        {
            ...
            } else printf_simple("Usage: append <file> <text>\n");
        }
        // touch
        else if (strncmp(cmd_line, "touch", 5) == 0)
        {
            char *p = cmd_line + 5;
            while (*p == ' ') p++;
            if (*p == '\0') printf_simple("Usage: touch <filename>\n");
            else {
                merge_path(p, arg_path);
                if (fcreate(arg_path) == 0) printf_simple("File created.\n");
                else printf_simple("Error: Failed to create file.\n");
            }
        }
        // rm / fdelete
        else if (strncmp(cmd_line, "fdelete", 7) == 0 || strncmp(cmd_line, "rm", 2) == 0)
        {
            ...
            while (*p == ' ') p++;
            if (*p == '\0') printf_simple("Usage: rm <filename>\n");
            else {
                merge_path(p, arg_path);
                if (fdelete(arg_path) == 0) printf_simple("File deleted successfully.\n");
                else printf_simple("Error: Could not delete file.\n");
            }
        }
        // run
