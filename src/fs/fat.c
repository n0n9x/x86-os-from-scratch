...
void fat_init()
{
    ide_read_sector(0, bpb_buf); // 读取引导扇区/第0扇区
    bpb = (fat_bpb_t *)bpb_buf;
...
{
    uint8_t fat_buf[512];
    // 一个fat条目2字节，cluster * 2为总字节偏移
    uint32_t fat_sector = fat_start_lba + (cluster * 2 / 512); // cluster所在扇区
    uint32_t entry_offset = (cluster * 2) % 512;               // cluster所在扇区的偏移量
...
int fat_read_file(const char *path, uint8_t *buffer, uint32_t limit)
{
    ...
    while (cluster >= 2 && cluster < 0xFFF8 && bytes_read < total_to_read)
    {
        ...
        if (remaining < 512) // 剩下的未读数据不足一个扇区，先全部读取再赋值
        {
...
int fat_find_entry_in_dir(uint16_t start_cluster, const char *name, fat_dirent_t *out_dirent, uint32_t *out_lba, int *out_idx)
{
    ...
    // 情况 A：处理根目录 (Root Directory) 根目录的目录项是连续存储
    if (start_cluster == 0)
    { // 逐个扇区遍历
...
int fat_path_to_dirent(const char *path, fat_dirent_t *out_dirent, uint32_t *out_lba, int *out_idx)
{
    ...
    while (*ptr)
    {
        ...
        if (*ptr == '/')
        {
            ...
            if (*ptr == '\0') // 是要找的目标文件、目录
            {
                if (out_dirent)
                    *out_dirent = current_entry;
                if (out_lba)
                    *out_lba = last_lba;
                if (out_idx)
                    *out_idx = last_idx;
...
void fat_set_cluster(uint16_t cluster, uint16_t next_value)
{
    ...
    ide_read_sector(fat_sector, fat_buf); // 读取数据
    *(uint16_t *)&fat_buf[ent_offset] = next_value;
    ide_write_sector(fat_sector, fat_buf); // 写回数据
}
...
uint16_t fat_allocate_cluster()
{
    ...
    for (uint32_t s = 0; s < bpb->sectors_per_fat; s++)
    {
        ...
        for (int i = 0; i < 256; i++) // 一个扇区有256个fat项
        {
            ...
            if (table[i] == 0x0000)
            {
                uint16_t cluster = (s * 256) + i; // 该扇区是空闲的
                // 找到后立即在 FAT 表中标记为已占用
...
int fat_create_file(const char *path)
{
    ...
    // 2. 检查文件是否已存在 (避免重复创建)
    if (fat_path_to_dirent(path, &temp, NULL, NULL) == 0)
        return -2;
    ...
    {
        if (fat_path_to_dirent(parent_path, &temp, NULL, NULL) != 0)
            return -3;
...
{
    return fat_write_file_at(path, data, len, 0);
}
